    ...
    <>
      ...
      >
        <div className="flex items-center justify-between p-6 border-b bg-gradient-to-r from-purple-600 to-pink-600">
          <h2 id="support-title" className="text-2xl font-bold text-white">Ask Support</h2>
          <button
            onClick={onClose}
        ...
        <div className="flex-1 overflow-y-auto p-6">
          <div className="mb-6 p-4 bg-purple-50 rounded-lg border border-purple-200">
            <p className="text-sm text-purple-900">
              Ask me about our policies, order status, returns, shipping, and more. Include your order ID for tracking queries.
            </p>
          </div>
              ...
              <div className="prose prose-sm max-w-none text-gray-900">
                {response.split(/(\[Q\d+\]|\[Order Status\])/).map((part, i) => {
                  if (part.match(/\[Q\d+\]|\[Order Status\]/)) {
                    return (
                      <span key={i} className="inline-flex items-center px-2 py-0.5 rounded bg-purple-100 text-purple-800 text-xs font-semibold ml-1">
                        {part}
                      </span>
                    );
                  }
